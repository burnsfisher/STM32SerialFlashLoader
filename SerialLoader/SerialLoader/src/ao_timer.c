...
/*
 * This code is extracted from Keith's ao_timer module, which also include a lot of initialization that
 * we don't need for this loader.  All we want to do here is get the processor clock running at 32MHz.
 * This code also gets the PLL running at a different speed for USB, but we don't need that right now.
 */


extern struct stm_pwr stm_pwr;
#define ao_arch_nop()  asm("nop")
#define AO_RCC_CFGR_PLLMUL (4) /* Unknown why it can't find the definitions in ao_flash_stm_pins.h*/
#define AO_RCC_CFGR_PLLDIV (2)
#define AO_HSE 1/*We do have a high speed external clock*/
void
ao_clock_init(void)
{
	uint32_t	cfgr;
