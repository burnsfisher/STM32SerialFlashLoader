...
/*
 * Fox note:
 * This file is used to avoid calling the ST libraries and thus reduce the amount of code
 * used in the most critical sections of Fox: Deploying the antennas.  It would be more
 * efficient to use this nice tight code everywhere, but we needed to use the libraries at
 * first in order to better understand the MCU and to be able to ask questions on the web.
 */
...
extern volatile struct stm_flash	stm_flash;

struct stm_scnscb {
       vuint32_t RESERVED0[1];
       vuint32_t ICTR;                    /*!< Offset: 0x004 (R/ )  Interrupt Controller Type Register      */
       vuint32_t ACTLR;                   /*!< Offset: 0x008 (R/W)  Auxiliary Control Register      */
};

extern volatile struct stm_scnscb stm_scnscb;
...
#define STM_FLASH_SR_OPTVERRUSR		12
...
#define STM_FLASH_PRGKEYR_PRGKEY1              ((uint32_t)0x8C9DAEBF) /*!< Flash program memory key1 */
#define STM_FLASH_PRGKEYR_PRGKEY2              ((uint32_t)0x13141516) /*!< Flash program memory key2: used with FLASH_PRGKEY2*/
#define STM_FLASH_PAGESIZE ((uint32_t)0x100)
#define STM_FLASH_PAGESIZE_WORDS ((uint32_t)STM_FLASH_PAGESIZE/4)
struct stm_rcc {
	vuint32_t	cr;
	vuint32_t	icscr;
	vuint32_t	cfgr;
...
#define STM_PWR ((struct stm_pwr) stm_pwr) //BF
